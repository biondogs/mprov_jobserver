...
class image_sync(JobServerPlugin):
    ...
    def load_config(self):
        if "mprov-webserver" not in self.js.jobmodules:
            print(
                "Configuration Error: you MUST run mprov-webserver on image-sync nodes!"
            )
            print("                   : image-sync nodes are SOURCE nodes for images!")
            print(
                "                   : and need a way to serve images! Job Module Halted!!!"
            )
            sys.exit(1)
    ...
    def handle_jobs(self):
        ...
        for image in self.imageList:
            ...
            response = self.js.session.get(
                self.js.mprovURL + "systemimages/" + image + "/"
            )
            if response.status_code == 404:
                ...
                print(
                    "Error: Image '"
                    + image
                    + "' was not found on the MPCC, removing locally."
                )
            ...
            if ourVersion != mpccVersion:
                ...
                with self.js.session.get(
                    self.js.mprovURL + "images/" + image, stream=True
                ) as remoteImage:
                    remoteImage.raise_for_status()
                    os.makedirs(self.imageDir + "/" + image, exist_ok=True)
                    with open(
                        self.imageDir + "/" + image + "/" + image + ".img", "wb"
                    ) as localFile:
                    ...
                    # grab the initramfs
                    with self.js.session.get(
                        self.js.mprovURL + "images/" + image + ".initramfs", stream=True
                    ) as remoteImage:
                        remoteImage.raise_for_status()
                        os.makedirs(self.imageDir + "/" + image, exist_ok=True)
                        with open(
                            self.imageDir + "/" + image + "/" + image + ".initramfs",
                            "wb",
                        ) as localFile:
                            for chunk in remoteImage.iter_content(chunk_size=8192):
                                localFile.write(chunk)
                    ...
                    # grab the kernel
                    with self.js.session.get(
                        self.js.mprovURL + "images/" + image + ".vmlinuz", stream=True
                    ) as remoteImage:
                        remoteImage.raise_for_status()
                        os.makedirs(self.imageDir + "/" + image, exist_ok=True)
                        with open(
                            self.imageDir + "/" + image + "/" + image + ".vmlinuz", "wb"
                        ) as localFile:
                            for chunk in remoteImage.iter_content(chunk_size=8192):
                                localFile.write(chunk)
            ...
            if self.js.id not in currJobServers:
                ...
                response = self.js.session.patch(
                    self.js.mprovURL + "systemimages/" + str(data["slug"]) + "/?addjs",
                    data=json.dumps(data),
                )
                print(f"Image {image} updated.")

    # Clean up any directories that are not in our imageList.
    print("Scanning for hanging images... " + self.imageDir)
        for entry in os.listdir(self.imageDir):
            ...
            if os.path.isdir(self.imageDir + "/" + entry):
                ...
                if not os.path.basename(entry) in self.imageList:
                    ...
                    print(
                        "Warn: Removed unknown image directory: "
                        + self.imageDir
                        + "/"
                        + entry
                    )
