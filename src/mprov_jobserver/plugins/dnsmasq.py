...
class dnsmasq(JobServerPlugin):
        ...
        if result:
            ...
            if buildIpxe:
                ...
                try:
                    ...
                    print(
                        "Error: unable to fetch ipxe source.  You may need to copy some files yourself."
                    )
                    return result
                ...
                with open("config/defaults/efi.h", "w") as f:
                    for line in lines:
                        if "DOWNLOAD_PROTO_HTTPS" in line.strip("\n"):
                            f.write(
                                f"#define DOWNLOAD_PROTO_HTTPS    /* Secure Hypertext Transfer Protocol */"
                            )
                        else:
                            f.write(f"{line}")
                try:
                ...
                try:
                    ...
                    print(
                        f"Error: Unable to copy bin-x86_64-efi/snponly.efi -> {self.tftproot}"
                    )
                ...
                try:
                    sh.cp(
                        [
                            ...
                            f"{self.tftproot}/snponly_ipv4.efi",
                        ]
                    )
                except:
                    print(
                        f"Error: Unable to copy bin-x86_64-efi/snponly.efi -> {self.tftproot}/snponly_ipv4.efi"
                    )
    ...
    def handle_jobs(self):
        ...
        if self.enableDNS:
            ...
            # print(jobquery)
            if (
                not self.js.update_job_status(
                    self.jobModule, 2, jobquery=jobquery + "&status=1"
                )
