...
class image_delete(JobServerPlugin):
    ...
    def handle_jobs(self):
        ...
        if not self.js.update_job_status(
        ...
        response = self.js.session.get(
            self.js.mprovURL + "jobs/?" + jobquery + "&status=2"
        )
        for job in response.json():
            ...
            if self.imageList is None or params["imageId"] in self.imageList:
                ...
                print("Error: Received image-delete job for image we don't host. HuH?")
                self.js.update_job_status(self.jobModule, 3, jobid=job["id"])
