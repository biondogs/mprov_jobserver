...
class repo_update(JobServerPlugin):
        ...
        for name in mods:
            try:
                mod = importlib.import_module(
                    f".repo_update_mod.{name}", "mprov_jobserver.plugins"
                )
    ...
    def handle_jobs(self):
        ...
        response = self.js.session.get(
            f"{self.js.mprovURL}jobs/?jobserver={str(self.js.id)}&module={self.jobModule}&status=2"
        )
        ...
        if not os.path.exists(self.repoDir):
            try:
                ...
                self.js.update_job_status(
                    self.module,
                    3,
                    jobquery="jobserver=" + self(self.js.id) + "&status=2",
                )
        ...
        for repo, jid in self.repoList.items():
            ...
            if response.status_code == 200:
                ...
                if repo["ostype"] == None:
                    print(
                        f"Error: Repo {repo['id']} doesn't have an OS type set.  Exiting."
                    )
                    self.js.update_job_status(
                        self.jobModule,
                        3,
                        jobid=jid,
                        jobquery="jobserver=" + str(self.js.id) + "&status=2",
                    )
                try:
                    print(f".repo_update_mod.{repo['ostype']}")
                    mod = importlib.import_module(
                        f".repo_update_mod.{repo['ostype']}", "mprov_jobserver.plugins"
                    )
                    update_klass = getattr(mod, "UpdateRepo")
                except BaseException as err:
                ...
                try:
                    ...
                    print(
                        f"Error: Unable to instantiate UpdateRepo class on {repo['ostype']}"
                    )
                    continue
